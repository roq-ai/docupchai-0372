...
import { getSiteOwners } from 'apiSdk/site-owners';
...
function DashboardPage() {
  ...
  const siteOwnerStat = useSWR<PaginatedInterface<SiteOwnerInterface>>(
    () => `/site-owners`,
    () => getSiteOwners({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Site Owners',
      result: siteOwnerStat?.data?.totalCount,
      isLoading: siteOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
